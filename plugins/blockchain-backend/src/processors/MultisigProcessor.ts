...
import { isBlockchainUser, isMultisigDeployment } from '../lib/types';

/**
 * Uses HTTP api to discover multisig owners
 *
 * TODO move to on-chain discovery?
 */
export class MultisigProcessor extends BlockchainProcessor {
  /**
   * Those Multisig components are explicitly declared in .yaml files
   */
  private isMultisigComponent(entity: Entity): boolean {
  ...
  async postProcessEntity?(
    ...
    if (this.isMultisigComponent(entity)) {
      ...
      // console.log(`fetching safe owners for ${entity.metadata.name}`);
